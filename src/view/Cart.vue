<template>
  <div>
            ...
            <ul class="cart-item-list">
              <li v-for="item in cartList">
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{item.productPrice | c('￥')}}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="cartNum('jian',item)">-</a>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{item.productNum * item.productPrice | c('￥')}}</div>
                </div>
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check"><!--全选-->
                <a href="javascipt:;" @click="allcheck"><!--全选-->
                  <!---->
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: {{totalPrice | c('￥')}}<span class="total-price"></span>
              </div>
              <div class="btn-wrap">
                <!--当按钮class为btn-dis按钮不可用,没有勾选商品的时候-->
                <a class="btn btn--red" v-bind:class="{'btn--dis':ckeck==0}" @click="checkOut">Checkout</a>
                <!--
                当勾选的商品数量等于0,空==0,所以不能结算,禁用了结算按钮,否则就可以跳转
                -->
              </div>
            </div>
          </div>
    ...
    <Modal :mdShow="delCartShow" v-on:close="closeModal">
      <p slot="message">是否删除此商品</p>
      <div slot="btnGroup">
        <a class="btn btn-m" @click="delComfirm">确定</a>
        <a class="btn btn-m" @click="delCartShow=false">取消</a>
      </div>
    </Modal>
    <!--删除底部-->
    <NavFooter></NavFooter>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...
      checkAllFlag(){
        return this.ckeck==this.cartList.length
      }
    },
    methods:{
      checkOut(){
      ...
      allcheck(){
        ...
        this.$axios.post('/users/allCheck',{checkAllFlag:flag}).then(res=>{
      ...
      editCart(item){
        ...
        this.$axios.post('/users/checkItem',{productId:item.productId,checked:item.checked}).then((res)=>{
          console.log('复选框更新成功了')
        })
      },
      // 购物车数量变化
      cartNum(tp,item){
      ...
      //模态框上的确认删除按钮
      delComfirm(){
        this.$axios.post('/users/deleteCart',{productId:this.productId}).then((res)=>{
      ...
      // 初始化购物车
      init(){
        this.$axios.post('/users/cartList').then((res)=>{
          if(res.data.status=='0'){
